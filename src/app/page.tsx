...
export default function Home() {
  ...
  const joinRoom = (e: React.FormEvent) => {
    ...
    // Parse full URLs: extract roomId from paths like /abc123?type=code
    try {
      const url = new URL(input);
      const pathParts = url.pathname.split("/").filter(Boolean);
      if (pathParts.length > 0) {
        router.push(`/${pathParts[0]}${url.search}`);
        return;
      }
    } catch {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] flex flex-col relative overflow-hidden">
      ...
      <div className="absolute top-3/4 left-1/2 w-64 h-64 bg-emerald-500/10 rounded-full blur-[128px]" />
      ...
      <div className="flex-1 flex items-center justify-center p-3 sm:p-4">
        <div className="w-full max-w-xl relative z-10">
              ...
              <>
                ...
                <form onSubmit={joinRoom} className="space-y-3 sm:space-y-4">
                  <input
                    ...
                    placeholder="Paste a link or room ID..."
          ...
          <div className="mt-8 sm:mt-12">
            <p className="text-center text-gray-500 text-xs uppercase tracking-wider mb-4">How it works</p>
            <div className="flex items-center justify-center gap-4 sm:gap-6 text-center">
              <div className="flex flex-col items-center gap-2">
                <div className="w-10 h-10 rounded-full bg-violet-500/20 flex items-center justify-center text-violet-400 font-bold text-sm">1</div>
                <span className="text-gray-400 text-xs sm:text-sm">Create</span>
              </div>
              <svg className="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
              </svg>
              <div className="flex flex-col items-center gap-2">
                <div className="w-10 h-10 rounded-full bg-cyan-500/20 flex items-center justify-center text-cyan-400 font-bold text-sm">2</div>
              ...
              <div className="flex flex-col items-center gap-2">
                <div className="w-10 h-10 rounded-full bg-emerald-500/20 flex items-center justify-center text-emerald-400 font-bold text-sm">3</div>
                <span className="text-gray-400 text-xs sm:text-sm">Collaborate</span>
              </div>
            </div>
          </div>
